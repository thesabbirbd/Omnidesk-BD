...
export default function Topics() {
  ...
  return (
    <div className="flex flex-col w-full min-h-full text-[color:var(--text-main)] gap-6 md:gap-8 pb-12">
      ...
      <div className="shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-80 shrink-0">
          ...
          <input
            ...
            className="w-full bg-[var(--bg-input)] border border-[var(--border-color)] text-[color:var(--text-main)] rounded-2xl py-3 pl-11 pr-4 focus:outline-none focus:border-purple-500/50 shadow-[var(--input-shadow)] text-sm font-medium transition-all"
          />
        </div>
      </div>
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer border select-none shrink-0 ${
                isSelected
                  ? 'bg-purple-500 text-white border-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.35)] scale-[1.02]'
                  : 'bg-[var(--bg-card)] text-[color:var(--text-muted)] border-[var(--border-color)] hover:text-[color:var(--text-main)] hover:border-purple-500/30'
              }`}
            >
              {cat}
            </button>
      ...
      {/* 6-Status Filter Bar */}
      <div className="shrink-0 flex flex-wrap items-center gap-2 w-full">
        {statuses.map((status) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer border select-none shrink-0 ${
                isSelected
                  ? `${conf.bg} ${conf.text} ${conf.border} shadow-[0_0_12px_rgba(168,85,247,0.3)] ring-1 ring-purple-500/40 scale-[1.02]`
                  : 'bg-[var(--bg-input)] text-[color:var(--text-muted)] border-[var(--border-color)] hover:text-[color:var(--text-main)] hover:border-purple-500/30'
              }`}
            >
              {status}
            </button>
      ...
      {filteredTopics.length === 0 ? (
        <div className="w-full py-16 flex flex-col items-center justify-center gap-3 text-center rounded-3xl bg-[var(--bg-card)] border border-[var(--border-color)] shadow-[var(--card-shadow)] p-8">
          <Layers className="text-[color:var(--text-muted)] opacity-50" size={48} />
          <h3 className="text-lg font-bold text-[color:var(--text-main)]">No Topics Found</h3>
          <p className="text-sm text-[color:var(--text-muted)] max-w-sm">
            No topics matched your search criteria. Try resetting your search or filter options.
          </p>
          <button
            onClick={() => { setSelectedCategory('All'); setSelectedStatus('ALL'); setSearchQuery(''); }}
            className="mt-2 px-4 py-2 rounded-xl text-xs font-bold bg-purple-500 hover:bg-purple-600 text-white transition-all cursor-pointer shadow-[0_4px_14px_rgba(168,85,247,0.4)]"
          >
            Reset All Filters
          </button>
        </div>
